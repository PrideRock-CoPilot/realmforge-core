//! MCP tool handler - dispatcher for all tool invocations.
...
pub async fn handle_tool(name: &str, args: Value, ctx: &ServiceContext) -> Result<Value, McpError> {
    let result = match name {
        ...
        "intake_answer" => {
            let args: tools::intake::IntakeAnswerArgs = serde_json::from_value(args)?;
            tools::intake::intake_answer(args, ctx).await
        }
        "intake_complete" => {
            let args: tools::intake::IntakeCompleteArgs = serde_json::from_value(args)?;
            tools::intake::intake_complete(args, ctx).await
        }
        "intake_ai_assist" => {
            let args: tools::intake::IntakeAiAssistArgs = serde_json::from_value(args)?;
            tools::intake::intake_ai_assist(args, ctx).await
        }
        "intake_generate_form" => {
            let args: tools::intake::IntakeGenerateFormArgs = serde_json::from_value(args)?;
            tools::intake::intake_generate_form(args, ctx).await
        }
