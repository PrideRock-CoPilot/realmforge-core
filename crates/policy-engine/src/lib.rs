//! RealmForge Policy Engine
//!
//! Authorization decisions, policy chains, and command-level policy checks.
//!
//! ## Module structure
//!
//! - [`types`] — Core types: `DenialCode`, `CorrectiveAction`, `PolicyDenial`, `PolicyDecision`
//! - [`authorizer`] — Scope-level authorization checks (`authorize_action`)
//! - [`chain`] — Policy chain evaluation (`PolicyChainResult`, `evaluate_policy_chain`, `PolicyChainEvaluator`)
//! - [`command`] — Command-specific policy (`authorize_command_action`, `authorize_command`)
...
pub use authorizer::authorize_action;
pub use chain::{evaluate_policy_chain, PolicyChainEvaluator, PolicyChainResult};
pub use command::{authorize_command, authorize_command_action};
pub use types::{CorrectiveAction, DenialCode, PolicyDecision, PolicyDenial};
