...
impl BoundedCommand {
    ...
    fn transition_to(&mut self, new_status: CommandStatus) -> Result<(), DomainError> {
        ...
        let allowed = match (from, &new_status) {
            (CommandStatus::Proposed, CommandStatus::Authorized) => true,
            (CommandStatus::Proposed, CommandStatus::Denied) => true,
            (CommandStatus::Authorized, CommandStatus::Applied) => true,
            (CommandStatus::Authorized, CommandStatus::Failed) => true,
            (CommandStatus::Authorized, CommandStatus::Denied) => true,
            _ => false,
        };
        if !allowed {
