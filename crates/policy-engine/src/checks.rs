//! Command-specific policy checks.
//!
//! Contains authorization logic for commands, including checks based on
//! command status and target validation.

use authority_domain::{ActorScope, CommandStatus};
use chrono::{DateTime, Utc};

use crate::decision::{CorrectiveAction, DenialCode, PolicyDecision};
use crate::evaluator::authorize_action;

/// Command-specific policy: check if a command can be authorized given its current status.
///
/// Checks that the command is not in a terminal state before running standard scope checks.
///
/// # Terminal States
/// - `Applied` - Command executed successfully, cannot re-authorize
/// - `Denied` - Command previously denied, propose a new command instead
/// - `Authorized` - Already authorized, call apply_command or deny_command instead
...
///
/// Checks:
/// 1. Scope-level authorization (expiry, staleness, action match, etc.)
/// 2. Target type validation against allowed targets
///
